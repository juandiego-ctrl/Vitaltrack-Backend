...
export class AntecedentesService {
  ...
  async guardarAntecedentes(datos: AntecedentesDto[]): Promise<{ accion: string; antecedente?: IAntecedentes; error?: string }[]> {
    const resultados = [];
  ...
  async buscarPorPacienteAntecedente(pacienteId: string): Promise<IAntecedentes[]> {
    return await this.antecedentesModel.find({ pacienteId }).exec();
  }
}
